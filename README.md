...
- Multi-project local persistence with IndexedDB, autosave, duplication, deletion, and automatic
  migration of the former single-plan `localStorage` format.
...
- Runtime overclocks, machine parallelism, amperage, and conservative probability bounds are
  modeled. Maintenance and pollution remain unreported until the oracle can export authoritative
  values for them.
- Chance outputs support a long-run average and a configurable Wilson lower-confidence bound.
  This is a planning safety bound, not a simulation of every individual operation.
...
- `src/lib/projects/` - IndexedDB project library and legacy persistence migration.
...
- Extend authoritative runtime coverage to maintenance, pollution, startup energy, and remaining
  dynamic Thaumcraft handlers.
